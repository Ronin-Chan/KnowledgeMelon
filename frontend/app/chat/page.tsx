...
const isDefaultConversationTitle = (title?: string | null) =>
  title === undefined ||
  title === null ||
  DEFAULT_CONVERSATION_TITLES.has(title.trim());

const deriveFallbackConversationTitle = (
  userMessage: string,
  ...
  return shortened.length > 42
    ? `${shortened.slice(0, 42).trim()}...`
    : shortened;
};
...
export default function ChatPage() {
      ...
      if (response.ok) {
        ...
        data.messages.forEach((message) => {
          if (
            ...
            message.retrieved_sources?.length
          ) {
  ...
  const handleDeleteConversation = async (conversationId: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `${t("chatDeleteConversationFailed")} (${response.status})`,
        );
      }
  ...
  const attachFilesToCurrentSession = async (files: File[]) => {
    ...
    try {
      ...
      setSelectedKnowledgeAttachmentIds((prev) => [
        ...new Set([...prev, ...previews.map((item) => item.id)]),
      ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const endpoint = shouldUseAdvancedEndpoint
        ? "/api/chat/rag"
        : "/api/chat";
      if (useRAG) {
        try {
          ...
          if (previewResponse.ok) {
            const previewData: { sources: RetrievedSource[] } =
      ...
      const msg =
        error instanceof Error
          ? error.message
          : t("chatDeleteConversationFailed");
      setErrorMessage(`${msg}`);
    } finally {
      setIsLoading(false);
        ...
        if (createdConversationThisTurn) {
          ...
          if (
            ...
            try {
              ...
              console.error(
                "Failed to update fallback conversation title:",
                error,
              );
            }
          }
    ...
    <div className="relative flex h-[100dvh] overflow-hidden bg-[radial-gradient(circle_at_top,rgba(0,0,0,0.04),transparent_28%),linear-gradient(to_bottom,rgba(250,250,250,1),rgba(255,255,255,1))] text-foreground dark:bg-[#0d0d0d] dark:[background-image:none]">
          ...
          <div className="mt-4 flex min-h-0 flex-1 flex-col px-1">
            ...
            <div className="grid grid-cols-3 gap-1.5 px-1">
              <button
                ...
                className={`flex min-h-16 w-full flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] leading-none transition-all cursor-pointer ${
                  useRAG
                ...
                {filteredConversations.length === 0 && searchQuery ? (
                  ...
                  filteredConversations.map((chat) => (
                    <div
                      key={chat.id}
                      title={chat.title}
                      className={`group flex items-start gap-2 rounded-2xl px-3 py-2.5 text-sm transition-all w-full ${
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-hidden bg-transparent">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
                {messages.map((message) => (
                  ...
                  >
                    {/* 头像 */}
                    <div
                      className={`shrink-0 flex h-8 w-8 items-center justify-center rounded-full text-xs font-medium ${
                        message.role === "user"
                          ? "bg-foreground text-background"
                    ...
                    >
                      <div
                        className={`inline-block max-w-5xl rounded-2xl px-3 py-2.5 text-left ${
                          message.role === "user"
          ...
          <div className="shrink-0">
            <AlertDialog
              ...
              onOpenChange={(open) =>
                !open && setPendingDeleteConversation(null)
              }
            >
              <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <div className="flex items-center justify-between gap-2 px-1">
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        <button
                          ...
                          onClick={() =>
                            setSelectedKnowledgeAttachmentIds(
                              pendingKnowledgeAttachments.map(
                                (item) => item.id,
                              ),
                            )
                          }
        ...
        <div className="shrink-0 px-4 pb-6 pt-2">
          <div className="max-w-5xl mx-auto">
            <div
              className={`rounded-[28px] border bg-background/95 shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_2px_2px_rgba(0,0,0,0.02),0_16px_48px_-32px_rgba(0,0,0,0.24)] transition-shadow hover:shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_6px_12px_rgba(0,0,0,0.06),0_20px_64px_-24px_rgba(0,0,0,0.28)] ${
                isDraggingFile
            ...
            >
              ...
              <div className="flex items-center gap-1 px-3 pt-2 pb-1">
                ...
                <button
                  ...
                  title={
                    isListening ? t("chatSpeechStop") : t("chatSpeechStart")
                  }
                  aria-label={
                    isListening ? t("chatSpeechStop") : t("chatSpeechStart")
                  }
